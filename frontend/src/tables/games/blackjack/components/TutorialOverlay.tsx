...
function Cards({ cards, theme, width = 56, flip }: { cards: string[]; theme: BjTheme; width?: number; flip?: boolean }) {
  return (
    <div className="flex">
      {cards.map((c, i) => (
...
function Total({ value, color }: { value: number | string; color: string }) {
  return (
    <span className="rounded-full px-3 py-1 font-display text-xl font-extrabold" style={{ background: 'rgba(0,0,0,0.6)', color }}>
      {value}
    </span>
  );
}
...
export default function TutorialOverlay({ state, you, theme, busy, onSkipVote, t }: Props) {
  ...
  const scenes: ReactNode[] = [
    ...
    <div key="goal" className="flex items-center gap-10">
      ...
      <span className="font-display text-4xl font-black text-white/60">VS</span>
      <div className="flex flex-col items-center gap-2">
        <Cards cards={['Td', '7c']} theme={theme} />
        <Total value={17} color="#EDF0F7" />
        <span className="text-sm font-bold text-white/70">{t('table.bj.dealer')}</span>
      </div>
    </div>,
    // 2. valeur des cartes
    <div key="values" className="flex items-end gap-6">
      {[
        ...
        <div key={card} className="flex flex-col items-center gap-2">
          <CardGlyph card={card} theme={theme} width={62} />
    ...
    <div key="hitstand" className="flex items-center gap-12">
      <div className="flex flex-col items-center gap-2">
        ...
        <span className="text-xs font-bold uppercase text-white/60">{t('table.bj.tuto.hitGood')}</span>
      </div>
      <div className="flex flex-col items-center gap-2 opacity-90">
        <Cards cards={['Th', '6c', 'Kd']} theme={theme} />
        <Total value={26} color={theme.danger} />
        <span className="text-xs font-bold uppercase" style={{ color: theme.danger }}>
    ...
    <div key="dealer" className="flex flex-col items-center gap-3">
      <Cards cards={['??', '8s']} theme={theme} width={64} />
      <div className="rounded-full px-4 py-1.5 font-display text-lg font-bold uppercase" style={{ background: 'rgba(0,0,0,0.6)', color: theme.feltText }}>
    ...
    <div key="score" className="flex items-center gap-5">
      <div className="flex items-center gap-2">
        <ChipGlyph value={100} theme={theme} size={48} />
        <span className="font-display text-2xl font-bold text-white/85">{t('table.bj.tuto.chipsWord')}</span>
      </div>
      <span className="font-display text-3xl text-white/50">+</span>
      <span className="font-display text-2xl font-bold" style={{ color: theme.gold }}>
        {t('table.bj.tuto.roundsWord')}
      </span>
      <span className="font-display text-3xl text-white/50">=</span>
      <span className="rounded-2xl px-4 py-2 font-display text-3xl font-black" style={{ background: `${theme.hudAccent}22`, color: theme.hudAccent }}>
    ...
    <div key="prime" className="flex items-center gap-8">
      <div className="flex flex-col items-center gap-1.5">
        <Cards cards={['Ks', 'Jd']} theme={theme} width={52} />
        <Total value={20} color={theme.gold} />
        <span className="text-xs font-bold text-white/70">{p1} · 2 {t('table.bj.tuto.cardsWord')}</span>
      </div>
      <div className="flex flex-col items-center gap-1.5 opacity-75">
        <Cards cards={['5h', '8c', '3s', '4d']} theme={theme} width={52} />
        <Total value={20} color="#EDF0F7" />
        <span className="text-xs font-bold text-white/60">{p2} · 4 {t('table.bj.tuto.cardsWord')}</span>
      </div>
      <span className="rounded-full px-4 py-2 font-display text-xl font-black uppercase" style={{ background: `${theme.gold}26`, color: theme.gold }}>
        +{state.config.prime}
      </span>
    </div>,
    // 7. doubler
    <div key="double" className="flex items-center gap-6">
      <div className="flex items-center gap-2">
        <ChipGlyph value={50} theme={theme} size={42} />
        <span className="font-display text-3xl font-black" style={{ color: theme.gold }}>
          x2
        </span>
      </div>
    ...
    <div key="bust" className="flex items-center gap-8">
      <div className="flex flex-col items-center gap-2">
        <div className="bj-hand-dimmed">
          <Cards cards={['Kh', '8d', '9c']} theme={theme} width={50} />
        </div>
        <Total value={27} color={theme.danger} />
      </div>
      <div className="flex items-center gap-2">
        {enabledJokers.slice(0, 2).map((type) => (
          <JokerGlyph key={type} type={type} theme={theme} width={52} t={t} compact />
        ))}
        <span className="ml-1 max-w-[220px] font-display text-base font-bold uppercase leading-tight" style={{ color: theme.hudAccent }}>
    ...
    <div key="jokers" className="flex items-center gap-6">
      <div className="flex gap-1.5">
        {Array.from({ length: 3 }, (_, i) => (
          <span key={i} className="h-8 w-6 rounded-[3px] border-2" style={{ background: theme.seatBg, borderColor: theme.hudAccent }} />
        ))}
      </div>
      <span className="max-w-[380px] text-center font-display text-lg font-bold uppercase leading-snug text-white/85">
        {t('table.bj.tuto.jokersAnytime')}
      </span>
    </div>,
    // 10. les six jokers
    <div key="six" className="grid grid-cols-3 gap-3">
      {enabledJokers.map((type, i) => (
        <div key={type} className="bj-pop flex items-center gap-2" style={{ animationDelay: `${i * 350}ms` }}>
          <JokerGlyph type={type} theme={theme} width={46} t={t} compact />
          <div>
            <div className="font-display text-sm font-bold uppercase" style={{ color: theme.hudAccent }}>
              {t(`table.bj.joker.${type}`)}
            </div>
            <div className="max-w-[190px] text-[11px] leading-tight text-white/65">{t(`table.bj.joker.${type}.desc`)}</div>
          </div>
        </div>
      ))}
    </div>,
    ...
    <div key="go" className="flex flex-col items-center gap-3">
      <div className="flex flex-wrap justify-center gap-2">
        {seats.map((s, i) => (
          <span
            key={s.playerId}
            className="bj-pop rounded-full px-4 py-1.5 font-display text-lg font-bold uppercase"
  ...
  return (
    <div className="pointer-events-auto absolute inset-0 z-40 flex flex-col items-center justify-center" style={{ background: 'rgba(3,5,12,0.93)' }}>
      {/* progression */}
      <div className="absolute left-1/2 top-8 w-[520px] -translate-x-1/2">
        ...
        <div className="mt-2 flex justify-between">
          {Array.from({ length: CHAPTERS }, (_, i) => (
            <span key={i} className="h-1.5 w-1.5 rounded-full" style={{ background: i <= chapter ? theme.hudAccent : 'rgba(255,255,255,0.18)' }} />
          ))}
        </div>
      </div>
      ...
      <div key={chapter} className="bj-chapter-in flex flex-col items-center gap-8 px-10">
        <div className="flex min-h-[190px] items-center justify-center">{scenes[chapter]}</div>
        <div className="max-w-[860px] text-center font-display text-3xl font-bold leading-snug text-white">
          {text(titles[chapter])}
        </div>
      </div>
      ...
      {seated && (
        <button
          className="absolute bottom-8 right-10 flex items-center gap-2.5 rounded-2xl border px-6 py-3.5 font-display text-lg font-bold uppercase active:scale-95"
          style={{
      ...
      {!seated && state.skipVotes.length > 0 && (
        <span className="absolute bottom-9 right-10 text-sm font-bold uppercase text-white/55">
          {t('table.bj.tuto.skip')} {state.skipVotes.length}/{seats.length}
        </span>
      )}
    </div>
