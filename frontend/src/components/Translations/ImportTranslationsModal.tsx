...
export default function ImportTranslationsModal({ onClose, onImported }: Props) {
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/import/contentful-translations`, {
        method: 'POST',
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (eventType === 'progress') {
              ...
              addLine(`Import terminé ! ${data.translations} clés importées.`, 'success');
              setDone(true);
              setImporting(false);
              toast.success(`${data.translations} traductions importées`);
              onImported();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
        <div className="flex items-center justify-between p-5 border-b">
          <h2 className="text-lg font-bold">Importer les traductions depuis Contentful</h2>
        ...
        <div className="p-5 space-y-4">
          {!importing && !done && lines.length === 0 && (
            <p className="text-sm text-gray-600">
              Cette action va importer toutes les clés de traduction (FR et EN) depuis Contentful.
              Les clés déjà existantes seront ignorées.
            </p>
          )}
          {error && (
