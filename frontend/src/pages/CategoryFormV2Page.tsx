...
export default function CategoryFormV2Page() {
  ...
  const [products, setProducts] = useState<ProductV2Data[]>([]);
  ...
  const [productModal, setProductModal] = useState<{
    open: boolean;
    editing: ProductV2Data | null;
    editIdx: number | null;
  }>({ open: false, editing: null, editIdx: null });

  const [searchOpen, setSearchOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [searchResults, setSearchResults] = useState<ProductV2Data[]>([]);
  ...
  const loadCategory = useCallback(async () => {
    ...
    try {
      const { data } = await api.get(`/api/menu-categories-v2/${id}`);
  ...
  useEffect(() => {
    api.get('/api/menu-categories-v2').then(({ data }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (form.color && !/^#[0-9A-Fa-f]{6}$/.test(form.color)) {
      toast.error('La couleur doit être au format hex #RRGGBB');
      return;
    }
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/api/menu-categories-v2/${id}`, payload);
        toast.success('Catégorie mise à jour');
      } else {
        const { data } = await api.post('/api/menu-categories-v2', payload);
        toast.success('Catégorie créée');
        navigate(`/contenus/carte-v2/category/${data.category.id}`, { replace: true });
      }
    } catch {
  ...
  const handleSearchExisting = useCallback(
    async (query?: string) => {
      ...
      try {
        ...
        const { data } = await api.get('/api/menu-products-v2', { params });
  ...
  const addExistingProduct = (p: ProductV2Data) => {
  ...
  const handleProductSave = async (data: ProductV2Data) => {
    try {
      if (data.id) {
        ...
        const { data: res } = await api.post('/api/menu-products-v2', data);
    ...
    <div className="max-w-4xl">
        ...
        <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => setProductModal({ open: true, editing: null, editIdx: null })}
            ...
            <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      {searchResults.map((p) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {alreadyAdded ? (
                              <span className="ml-2 px-2 py-1 text-xs text-gray-400">Déjà ajouté</span>
                            ) : (
                              <button
                                type="button"
              ...
              {searchResults.length === 0 && !searchLoading && (
                <p className="text-sm text-gray-400">
                  {searchQuery ? 'Aucun produit ne correspond à la recherche' : 'Aucun produit disponible'}
                </p>
              )}
            </div>
          )}
              ...
              {products.map((p, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      type="button"
                      onClick={() => setProductModal({ open: true, editing: p, editIdx: idx })}
      ...
      {productModal.open && (
        <ProductModalV2
          ...
          onClose={() => setProductModal({ open: false, editing: null, editIdx: null })}
        />
      )}
    </div>
  );
