...
--
-- Le `theme` de chaque question est un VRAI thème de contenu (il s'affiche en
-- gros titre à l'annonce) : le cas testé, lui, se lit dans les badges de
-- l'annonce, qui affichent déjà le type (QCM / Estimation / Réponse libre) et
-- le média (audio / vidéo / image). La matrice ci-dessus donne la position de
-- chaque cas.
--
...
-- On relie par le TEXTE de la question, unique dans ce quiz (les thèmes, eux,
-- se répètent : trois questions Disney par exemple).
INSERT INTO quiz_questions (quiz_id, question_id, position)
SELECT (SELECT id FROM new_quiz), ins.id, src.pos
FROM ins JOIN src ON src.question = ins.question;
