# edge-master.ps1 - Lanceur du kiosque Edge sur les PC MASTER des tables (TABLExx-1)
#
# ROLE
#   Dernier maillon local de la chaine d'affichage d'une table : c'est lui, et lui
#   seul, qui decide quelle URL Edge affiche. Deploye sur le poste sous le nom
#   C:\INVADER\SCRIPTS\edge.ps1
...
#
# CE QU'IL FAIT
#   1. demarre le serveur web local (php-cgi + nginx) qui sert http://localhost
#   2. clone les deux ecrans (DisplaySwitch /clone)
#   3. lance Edge en kiosque plein ecran sur l'URL calculee
#   4. amorce le cache de l'ancienne interface (menu.php puis game.php)
#   5. boucle a vie : relance Edge s'il meurt, et applique une URL forcee si le
#      back-office en demande une
#
# QUELLE URL EST AFFICHEE
#   Par defaut : le contenu de C:\INVADER\kioskURL.txt, soit l'ancienne interface
#   PHP servie en local (ex. http://localhost?type=table).
...
#   bouton "Basculer vers interface V2" de la page Gestion du bar).
...
#   poste sur kioskURL.txt. C'est volontaire, c'est ce que promet le back-office.
#   Le parametre hostname n'est ajoute que s'il manque, avec le bon separateur
#   (? si l'URL n'a pas encore de query string, & sinon) : c'est lui qui dit a
#   l'interface quelle table elle est.
#
...
# L'interface a besoin de savoir quelle table elle est. On n'ajoute le parametre
...
# Amorcage du cache de l'ancienne interface. En try/catch : si le serveur local
# n'est pas encore pret, cela ne doit pas empecher la boucle de relance de
# demarrer, sinon le kiosque n'est plus surveille du tout.
Start-Sleep -Seconds 10
foreach ($page in @("menu.php", "game.php")) {
    try {
        Invoke-RestMethod -Uri ("http://" + $env:COMPUTERNAME + "/" + $page) -Method Get -TimeoutSec 30 | Out-Null
    } catch { }
    Start-Sleep -Seconds 10
}
